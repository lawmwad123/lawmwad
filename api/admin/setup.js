...
async function handleMigrate(req, res) {
  ...
  try {
    await sql`CREATE TABLE IF NOT EXISTS users (
      ...
      role TEXT NOT NULL DEFAULT 'member' CHECK (role IN ('admin','director','lead','member','viewer')),
    ...
    await sql`CREATE TABLE IF NOT EXISTS team_members (
      ...
      name TEXT NOT NULL, email TEXT NOT NULL, phone TEXT, photo_url TEXT,
      division TEXT NOT NULL CHECK (division IN ('software','robotics','mechanics','electronics')),
      role TEXT NOT NULL, title TEXT, bio TEXT, skills TEXT[],
      join_date DATE, status TEXT DEFAULT 'active' CHECK (status IN ('active','on_leave','alumni')),
    ...
    await sql`CREATE TABLE IF NOT EXISTS projects (
      ...
      division TEXT NOT NULL CHECK (division IN ('software','robotics','mechanics','electronics','cross-division')),
      ...
      budget_allocated NUMERIC(12,2) DEFAULT 0, budget_spent NUMERIC(12,2) DEFAULT 0,
      lead_id UUID REFERENCES team_members(id) ON DELETE SET NULL,
      image_url TEXT, repository_url TEXT,
      created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW()
    )`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS project_members (
      ...
      role TEXT DEFAULT 'contributor', assigned_at TIMESTAMP DEFAULT NOW(),
    ...
    await sql`CREATE TABLE IF NOT EXISTS milestones (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      project_id UUID REFERENCES projects(id) ON DELETE CASCADE,
      title TEXT NOT NULL, description TEXT, due_date DATE, completed_at TIMESTAMP,
    ...
    await sql`CREATE TABLE IF NOT EXISTS equipment (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      name TEXT NOT NULL,
      category TEXT NOT NULL CHECK (category IN ('drone','3d_printer','sdr','sensor','tool','computer','other')),
      description TEXT, serial_number TEXT, location TEXT,
      status TEXT DEFAULT 'available' CHECK (status IN ('available','in_use','maintenance','retired','lost')),
      condition TEXT DEFAULT 'good' CHECK (condition IN ('new','good','fair','poor','broken')),
      purchase_date DATE, purchase_cost NUMERIC(10,2), photo_url TEXT,
      assigned_to UUID REFERENCES team_members(id) ON DELETE SET NULL, notes TEXT,
      created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW()
    )`;
    results.push('equipment');

    await sql`CREATE TABLE IF NOT EXISTS equipment_bookings (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      equipment_id UUID REFERENCES equipment(id) ON DELETE CASCADE,
      booked_by UUID REFERENCES team_members(id) ON DELETE CASCADE,
      project_id UUID REFERENCES projects(id) ON DELETE SET NULL,
      start_time TIMESTAMP NOT NULL, end_time TIMESTAMP NOT NULL, purpose TEXT,
      status TEXT DEFAULT 'confirmed' CHECK (status IN ('pending','confirmed','cancelled','completed')),
      created_at TIMESTAMP DEFAULT NOW()
    )`;
    results.push('equipment_bookings');

    await sql`CREATE TABLE IF NOT EXISTS maintenance_log (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      equipment_id UUID REFERENCES equipment(id) ON DELETE CASCADE,
      ...
      performed_at TIMESTAMP DEFAULT NOW(), next_maintenance DATE
    )`;
    results.push('maintenance_log');

    await sql`CREATE TABLE IF NOT EXISTS applications_v2 (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      name TEXT NOT NULL, email TEXT NOT NULL, phone TEXT, lab TEXT NOT NULL, level TEXT NOT NULL,
      about TEXT, resume_url TEXT, portfolio_url TEXT,
      status TEXT DEFAULT 'new' CHECK (status IN ('new','screening','interview','trial','accepted','rejected','withdrawn')),
      stage_notes JSONB DEFAULT '[]',
      reviewer_id UUID REFERENCES team_members(id) ON DELETE SET NULL,
      rating INT CHECK (rating >= 1 AND rating <= 5), source TEXT DEFAULT 'website',
      created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW()
    )`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS publications (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      title TEXT NOT NULL, abstract TEXT, authors TEXT[] NOT NULL,
      publication_type TEXT NOT NULL CHECK (publication_type IN ('paper','report','thesis','patent','blog','presentation')),
      venue TEXT, published_date DATE, doi TEXT, url TEXT, pdf_url TEXT,
      project_id UUID REFERENCES projects(id) ON DELETE SET NULL,
    ...
    await sql`CREATE TABLE IF NOT EXISTS documents (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      title TEXT NOT NULL, description TEXT, file_url TEXT NOT NULL, file_type TEXT, file_size INT,
      category TEXT NOT NULL CHECK (category IN ('report','proposal','datasheet','sop','meeting_notes','policy','other')),
      project_id UUID REFERENCES projects(id) ON DELETE SET NULL,
      uploaded_by UUID REFERENCES team_members(id) ON DELETE SET NULL,
      is_public BOOLEAN DEFAULT false, tags TEXT[],
      created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW()
    )`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS expenses (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      project_id UUID REFERENCES projects(id) ON DELETE SET NULL,
      category TEXT NOT NULL CHECK (category IN ('equipment','materials','software','travel','services','other')),
      description TEXT NOT NULL, amount NUMERIC(12,2) NOT NULL, currency TEXT DEFAULT 'USD',
      receipt_url TEXT, incurred_date DATE NOT NULL,
    ...
    await sql`CREATE TABLE IF NOT EXISTS audit_log (
      ...
      changes JSONB, ip_address TEXT, user_agent TEXT,
      created_at TIMESTAMP DEFAULT NOW()
    )`;
    ...
    return res.status(200).json({ message: 'Migration completed successfully', tables: results });
  } catch (err) {
...
async function handleSeed(req, res) {
  ...
  try {
    ...
    if (existing.length > 0) return res.status(409).json({ error: 'Admin user already exists' });
    ...
    return res.status(201).json({ message: 'Admin user created successfully', user });
  } catch (err) {
